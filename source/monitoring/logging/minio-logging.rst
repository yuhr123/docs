...
MinIO always logs regular operations to the system console. MinIO also
supports publishing both regular logs and audit logs to any
generic HTTP endpoint. Audit logs are granular descriptions of
...
.. _minio-logging-publish-logs:

Publish Logs to HTTP Target
---------------------------

You can configure a new HTTP webhook target to which MinIO publishes logs using
either environment variables *or* by setting runtime configuration settings:

.. tabs::

   .. tab:: Environment Variables

      MinIO supports specifying the log HTTP webhook target and associated
      configuration settings using :ref:`environment variables
      <minio-sever-envvar-logging-regular>`.

      The following example code sets *all* environment variables related to
      configuring a log HTTP webhook target. The minimum *required* variables
      are:
      ...
      - Replace ``<IDENTIFIER>`` with a unique descriptive string for the 
        ...
        to review the currently configured log HTTP webhook targets.
   ...
   .. tab:: Configuration Settings
      ...
      MinIO supports adding or updating log HTTP webhook targets on a MinIO
      ...
      a log HTTP webhook target. The minimum *required* setting is 
...
Publish Audit Logs to HTTP Target
---------------------------------

You can configure a new HTTP webhook target to which MinIO publishes audit logs
using either environment variables *or* by setting runtime configuration
settings:
...
.. tabs::
   ...
   .. tab:: Environment Variables
      ...
      MinIO supports specifying the audit log HTTP webhook target and associated
      configuration settings using :ref:`environment variables
      <minio-sever-envvar-logging-audit>`.

      The following example code sets *all* environment variables related to
      configuring a audit log HTTP webhook target. The minimum *required* variables
      are:
      ...
      - Replace ``<IDENTIFIER>`` with a unique descriptive string for the 
        ...
        to review the currently configured audit log HTTP webhook targets.
      ...
      - Replace ``cert.pem`` and ``cert.key`` with the public and private key
        ...
        Omit for endpoints which di not require clients to present TLS
        certificates.
   ...
   .. tab:: Configuration Settings
      ...
      MinIO supports adding or updating audit log HTTP webhook targets on a MinIO
      deployment using the :mc-cmd:`mc admin config set` command and the
      :mc-conf:`audit_webhook` configuration key. You must restart the
      MinIO deployment to apply any new or updated configuration settings.

      The following example code sets *all* settings related to configuring
      a audit log HTTP webhook target. The minimum *required* setting is 
