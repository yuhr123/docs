...
- For alerts, use Prometheus :prometheus-docs:`Alerting Rules
  <prometheus/latest/configuation/alerting_rules/>` and the
...
Replace ``http://minio.example.com`` with the hostname for the MinIO
node.
...
MinIO publishes the following metrics. Each metric includes a label for
...
.. metric:: minio_bucket_replication_failed_count
   ...
   Total number of replication foperations failed for this bucket.
...
<prometheus/latest/configuation/alerting_rules/>` based on the collected MinIO
